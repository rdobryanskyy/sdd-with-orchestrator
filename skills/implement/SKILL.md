...
**Orchestrated mode.** Under `/sdd:orchestrate` a `stage-runner` cannot spawn agents, so the stage splits: the runner does steps 1–7 (`phase=plan`, returns the run-plan), the **orchestrator** acts as the lead for step 8–9 (RED → GREEN → gate → commit per task, via `sdd:test-author` / `sdd:implementer`), and a runner does step 10 (`phase=finalize`) → [`../orchestrate/references/implement-lead.md`](../orchestrate/references/implement-lead.md). Every TDD, lane, escalation and gate rule below still holds.
