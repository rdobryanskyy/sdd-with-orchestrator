...
5. **Write the review record.** `docs/features/<slug>/_review/review-<date>.md` (if a record for that date already exists — a second round the same day — suffix `-r<n>`, never overwrite): scope (diff stat), findings with verdicts, and the gate result (`PASS` / `CHANGES REQUESTED`).
