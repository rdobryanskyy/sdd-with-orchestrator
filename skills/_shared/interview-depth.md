...
“Depth dial” is one per-run control for a skill: **easy / medium / hard**.

- **easy** — the skill makes most decisions using sensible defaults, asks only about irreversible or high-risk choices, and **lists its assumptions** so you can veto them. Less analysis; diagrams are written with one summary and no question per item.
- **medium** — the current balanced Socratic pass (default).
- **hard** — work through **every** decision; every `AskUserQuestion` foregrounds the trade-off; use the full set of idea analyses (research / approaches / perspectives / devil's advocate); confirm every diagram with prose; investigate edge cases more deeply.

Completeness (coverage of every AC) **does not depend** on the level—easy still covers all ACs; it simply asks less about *how*.
