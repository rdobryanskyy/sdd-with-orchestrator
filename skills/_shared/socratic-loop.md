...
The “Socratic loop” is a dialogue with the user about each **section/group** of an artifact:

1. The skill **renders the full section at once** and numbers its decisions (the big picture before details).
2. It asks about **one decision at a time** through `AskUserQuestion` (wording: [ask-style.md](./ask-style.md)).
3. The user chooses one of **four actions**: **Approve** / **Edit** / **Save as Open Question** / **Drop**.
4. The skill **applies the transition** in memory, maintains an **edits log**, and writes to disk and commits only at the end of the section.
5. A separate clean-context critic catches internal contradictions between sections ([critic.md](./critic.md)).
...
> **Plain-English summary.** The four actions for every decision are **Approve** / **Edit** /
> **Save as Open Question** / **Drop**. `Cancel` and `Reject` are synonyms for Drop.
