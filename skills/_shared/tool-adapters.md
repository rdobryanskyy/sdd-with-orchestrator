...
| Continue a finished subagent (`SendMessage` — the orchestrator resuming a `stage-runner` with `SDD_ANSWERS` / `SDD_REPORTS`) | native | not available — use the **replay** fallback in [`orchestration.md`](./orchestration.md) (fresh runner + answer sheet) | same as Codex |
| `/sdd:orchestrate` (orchestrator + `stage-runner` agents) | native | `$sdd-orchestrate`; runners dispatched as the `sdd-stage-runner` custom agent, replay-only; if agents are out of reach, the orchestrator runs each stage inline in sequence, still answering from the decision policy | same as Codex (`sdd-orchestrate`, `sdd-stage-runner`) |
| Computer use / a driven browser (the `visual-tester` in `visual-test`) | Claude in Chrome, the built-in browser, or any computer-use / Playwright MCP in the agent's tools | a Playwright script via the shell for `web-frontend` (headless Chromium); `desktop-app` / `mobile-app` need a computer-use MCP configured in the host, else the surface is `VISUAL BLOCKED` | same as Codex |
